...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result_patient->num_rows > 0) {
        ...
        $cookieExpiration = time() + (5 * 60);
        setcookie('error_message', "Nombre de usuario o contraseña incorrectos", $cookieExpiration, "/");
...
function setUserCookie($userId, $userType, $userName,$registration_number) {
    // Establece un tiempo de expiración para la cookie (por ejemplo, 5 horas)
   // $cookieExpiration = time() + (5 * 60 * 60);
   $cookieExpiration = time() + (5 * 60);
